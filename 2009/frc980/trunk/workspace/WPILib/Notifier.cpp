...
void Notifier::UpdateAlarm()
{
    ...
    else
    {
        ...
        timerQueueHead->m_expirationTime =
            GetClock() + timerQueueHead->m_period;
        talarm->
            writeTriggerTime((UINT32)
                             (timerQueueHead->m_expirationTime * 1e6),
                             &status);
...
void Notifier::DeleteFromQueue()
{
    if (m_queued)
    {
        ...
        {
            for (Notifier * n = timerQueueHead; n != NULL;
                 n = n->m_nextEvent)
            {
