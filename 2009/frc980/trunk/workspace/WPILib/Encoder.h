 ...
 * direction. The output of the QuadEncoder class is an integer that can
 * count either up or down, and can go negative for reverse direction
 * counting. When creating QuadEncoders, a direction is supplied that
 * changes the sense of the output to make code more readable if the
 * encoder is mounted such that forward movement generates negative
 * values. Quadrature encoders have two digital outputs, an A Channel and
...
{
  public:
    Encoder(UINT32 aChannel, UINT32 bChannel,
            bool reverseDirection = false);
    Encoder(UINT32 aSlot, UINT32 aChannel, UINT32 bSlot, UINT32 _bChannel,
            bool reverseDirection = false);
