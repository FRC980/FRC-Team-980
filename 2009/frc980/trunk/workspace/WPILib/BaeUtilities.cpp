...
/**
 * Debug print to a file and/or a terminal window.  Call like you would
 * call printf.  Set functionName in the function if you want the correct
 * function name to print out.  The file line number will also be printed.
 * @param tempString The format string.
 */
 ...
 */
float NormalizeToRange(float normalizedValue, float minRange,
                       float maxRange)
{
...
/**
 * @brief Calculate sine wave increments (-1.0 to 1.0).  The first time
 * this is called, it sets up the time increment. Subsequent calls will
 * give values along the sine wave depending on current time. If the wave
 * is stopped and restarted, it must be reinitialized with a new "first
 * call".
 *
