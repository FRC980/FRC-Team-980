...
* @brief Find the largest particle that meets a criteria 
* @param binaryImage Image to inspect
* @param largestParticleIndex Index of the largest particle DWD
* @param rect area to search
* @return 0 = error
*/
...
{
    return GetLargestParticle(binaryImage, largestParticleIndex,
                              IMAQ_NO_RECT);
}
                       ...
                       Rect rect)
{
    ...
    *largestParticleIndex = 0;  // points to caller-provided variable
    ...
    {
        return success;
    }                           // unsuccessful if zero particles found
...
int FindColor(FrcHue color, ParticleAnalysisReport * trackReport)
{
    ...
    success =
        FindColor(IMAQ_HSL, &td.hue, &td.saturation, &td.luminance,
                  trackReport);
    if (!success)
    {
        dprintf(LOG_INFO, "did not find color - errorCode= %i",
                GetLastVisionError());
        return success;
    }
...
int FindColor(ColorMode mode, const Range * plane1Range,
              ...
              ParticleAnalysisReport * trackReport)
{
    return FindColor(mode, plane1Range, plane2Range, plane3Range,
                     trackReport, NULL);
}
              ...
              ColorReport * colorReport, Rect rect)
{
    ...
    {
        errorCode = GetLastVisionError();
        DPRINTF(LOG_DEBUG, "Error after GetLargestParticle = %i  %s ",
                errorCode, GetVisionErrorText(errorCode));
        frcDispose(funcName, cameraImage, histImage, NULL);
        return success;
    }
    ...
    /* if no particles found we can quit here */
    //      if (largestParticleIndex == 0)  {               // if no particles found we wouldn't be here :)

    /* 
     * Fill in report information for largest particle found
     */
    success =
        frcParticleAnalysis(cameraImage, largestParticleIndex, trackReport);
    ...
    if (colorReport != NULL)
    {
        ...
        int  numParticles;
        success =
            frcParticleFilter(cameraImage, cameraImage, &criteria, 1,
                              options, rect, &numParticles);
        if (!success)
        {
            DPRINTF(LOG_INFO, "frcParticleFilter errorCode %i",
                    GetLastVisionError());
        }
    ...
    //      }       else    {       // if no particles found we wouldn't have gotten this far
    /* no particles found */
...
TrackingThreshold GetTrackingData(FrcHue hue, FrcLight light)
{
    ...
    case FLUORESCENT:
        ...
        if (hue == PINK)
            trackingData.saturation.minValue = 80;
        if (hue == PINK)
            trackingData.luminance.minValue = 60;
        if (hue == PINK)
            trackingData.luminance.maxValue = 155;
        break;
    case PASSIVE_LIGHT:
...
void PrintReport(ParticleAnalysisReport * myReport)
{
    ...
    dprintf(LOG_INFO,
            "particle analysis:\n    %s%i  %s%i\n    %s%d\n    %s%i  %s%i\n    %s%g  %s%g\n    %s%g\n    %s%i  %s%i\n    %s%i  %s%i\n",
            "imageHeight = ", myReport->imageHeight, "imageWidth = ",
            myReport->imageWidth, "imageTimestamp = ",
            myReport->imageTimestamp, "center_mass_x = ",
            myReport->center_mass_x, "center_mass_y = ",
            myReport->center_mass_y, "center_mass_x_normalized = ",
            myReport->center_mass_x_normalized,
            "center_mass_y_normalized = ",
            myReport->center_mass_y_normalized, "particleArea = ",
            myReport->particleArea, "boundingRectangleTop = ",
            myReport->boundingRect.top, "boundingRectangleLeft = ",
            myReport->boundingRect.left, "boundingRectangleHeight = ",
            myReport->boundingRect.height, "boundingRectangleWidth = ",
            myReport->boundingRect.width);
...
void PrintReport(TrackingThreshold * myReport)
{
    ...
    dprintf(LOG_INFO,
            "\n    %s%i  %s%i\n    %s%i %s%i\n    %s%i  %s%i\n -------",
            "hueMin = ", myReport->hue.minValue, "hueMax = ",
            myReport->hue.maxValue, "satMin = ",
            myReport->saturation.minValue, "satMax = ",
            myReport->saturation.maxValue, "lumMin = ",
            myReport->luminance.minValue, "lumMax = ",
            myReport->luminance.maxValue);
