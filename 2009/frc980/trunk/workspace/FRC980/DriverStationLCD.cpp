    ...
    , m_textBufferSemaphore(NULL)
{
    ...
    *((UINT16 *) m_textBuffer) = kFullDisplayTextCommand;
    ...
    m_textBufferSemaphore =
        semMCreate(SEM_DELETE_SAFE | SEM_INVERSION_SAFE);
...
void DriverStationLCD::Clear(void)
{
    Synchronized sync(m_textBufferSemaphore);
    memset(m_textBuffer, ' ', USER_DS_LCD_DATA_SIZE);
}
 ...
 * @param startingColumn The column to start printing to.  This is a
 * 1-based number.
                              ...
                              const char *writeFmt, ...)
{
    ...
    va_start(args, writeFmt);
    {
        ...
        INT32 length =
            vsnprintf(lineBuffer, kLineLength + 1, writeFmt, args);
        memcpy(m_textBuffer + start + line * kLineLength + sizeof(UINT16),
               lineBuffer, std::min(maxLength, length));
    }
