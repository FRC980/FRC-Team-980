...
* @return 0 = error
*/
int GetLargestParticles(Image * binaryImage, ImageHits * hitReport,
                        Rect rect, int numberHitsRequested)
{
    //char funcName[]="GetLargestParticles";

    // list to contain all particles within area sorted by size
    HitList *hitsInArea = new HitList();
    int  i;
    ...
    {
        success =
            imaqMeasureParticle(binaryImage, i, 0, IMAQ_MT_AREA,
                                &particleArea);
        if (!success)
        {
            return success;
        ...
        // see if is in the right area and large enough to be considered a
        // possible target
        if (InArea(binaryImage, i, rect)
            && (particleArea >= FRC_MINIMUM_PIXELS_FOR_TARGET))
        {
...
/**
 * @brief Search for color particles. Supports IMAQ_IMAGE_HSL and
 * IMAQ_IMAGE_RGB.
                  ...
                  ImageHits * hitReport, Rect rect, int numberHitsRequested)
{
     ...
     * larger # takes more processing time but can eliminate failure due
     * to interference for instance, a red or green area in the stands or
    ...
    /* get image from camera - if the camera has not finished
     * initializing, this will fail */
    ...
    success =
        frcColorThreshold(cameraImage, cameraImage, mode, plane1Range,
                          plane2Range, plane3Range);
    if (!success)
    {
        errorCode = GetLastVisionError();
        DPRINTF(LOG_DEBUG, "Error = %i  %s ", errorCode,
    ...
    success =
        GetLargestParticles(cameraImage, hitReport, rect,
                            numberHitsRequested);
    if (!success)
    {
        errorCode = GetLastVisionError();
        DPRINTF(LOG_DEBUG, "Error after GetLargestParticles=%i  %s ",
                errorCode, GetVisionErrorText(errorCode));
        frcDispose(funcName, cameraImage, NULL);
    ...
    for (int i = 0; i < hitReport->numberOfHits; ++i)
    {
        ...
        success =
            frcParticleAnalysis(cameraImage, hitReport->indices[i],
                                &hitReport->pars[i]);
    }
    /* clean up */
    frcDispose(funcName, cameraImage, NULL);
 ...
 * @param center1 Center pixel of first partical (x for vertical
 * alignment, y for horizontal)
 * @param center2 Center pixel of second partical (x for vertical
 * alignment, y for horizontal)
 * @param dimension1 of first partical (width for vertical alignment,
 * height for horizontal)
 * @param dimension2 of second partical (width for vertical alignment,
 * height for horizontal)
...
bool Aligned(int center1, int center2, int dimension1, int dimension2)
{
    ...
    //dimensions (widths or heights) should be nearly the same. If they
    //are different, most likely there is glare or incorrect color
    //specification
    DPRINTF(LOG_DEBUG, "Not Aligned Centers: %i %i Widths: %i  %i", center1,
            center2, dimension1, dimension2);
    return false;
}

/**
* @brief Check if two values are approximately adjacent
* @param value1 Center pixel of first partical (x for vertical alignment,
* y for horizontal)
* @param value2 Center pixel of second partical (x for vertical alignment,
* y for horizontal)
...
{
    if ((area2 < (area1 * FRC_SIZE_FACTOR))
        && (area1 < (area2 * FRC_SIZE_FACTOR)))
    {
        return true;
    }
    DPRINTF(LOG_DEBUG, "Targets too different in size %g  %g", area1,
            area2);
    return false;
}
                  ...
                  ParticleAnalysisReport * par2, int numberHits)
{
    ...
    // search for the largest particles of the first color; get several
    // particles in case the first one turns out not to be a target
    ...
    for (int i = 0; i < color1Hits.numberOfHits; ++i)
    {
        ...
        for (int j = 0; j < color2Hits.numberOfHits; ++j)
        {
            ...
            if (secondTrackReport->particleToImagePercent <
                FRC_COLOR_TO_IMAGE_PERCENT)
            {
                DPRINTF(LOG_DEBUG, "Second target too small: %g",
                        sizeInImage);
                break;
            }
            ...
            case ABOVE:         //is second color above first color?
                if (secondTrackReport->center_mass_y <
                    firstTrackReport->center_mass_y)
                {
                                         ...
                                         secondTrackReport->particleArea))
                    {
                        ...
                        memcpy(par1, firstTrackReport,
                               sizeof(ParticleAnalysisReport));
                        memcpy(par2, firstTrackReport,
                               sizeof(ParticleAnalysisReport));
                        return true;
                    }
                }
            ...
            case RIGHT:     //is second color to the right of first color?
                if (secondTrackReport->center_mass_x >
                    firstTrackReport->center_mass_x)
                {
    ...
    DPRINTF(LOG_DEBUG, "%s size %g and %s size %g NOT IN CORRECT POSITION",
            td1.name, firstTrackReport->particleArea, td2.name,
            secondTrackReport->particleArea);
...
void PrintReport(ImageHits * myReport)
{
    ...
    dprintf(LOG_INFO, "ImageHit Report: %i hit\(s)      ",
            myReport->numberOfHits);
