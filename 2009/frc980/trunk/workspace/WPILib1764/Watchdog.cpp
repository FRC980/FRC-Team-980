 ...
 * starves to death.
 *
 * @param expiration The number of seconds before starvation following a
 * meal (watchdog starves if it doesn't eat this often).
 */
 ...
 * watchdog active, and hence the dangerous parts (motor outputs, etc.)
 * can keep functioning.
 * When disabled, the watchdog is immortal and will remain active even
 * without being fed.  It will also ignore any kill commands while
 * disabled.
 *
 ...
 * It is still possible that the network communications is not allowing
 * the system to run, but you can check this to make sure it's not your
 * fault.  Check IsSystemActive() for overall system status.
 *
