...
void Solenoid::InitSolenoid()
{
    ...
    allocated->Allocate(SlotToIndex(m_chassisSlot) * kSolenoidChannels +
                        m_channel - 1);
 ...
 */
Solenoid::Solenoid(UINT32 channel):m_chassisSlot(GetDefaultSolenoidModule()),
m_channel(channel)
{
    InitSolenoid();
}
 ...
 * @param channel The channel on the module to control.
 */
Solenoid::Solenoid(UINT32 slot, UINT32 channel):m_chassisSlot(slot),
m_channel(channel)
{
    InitSolenoid();
}
...
Solenoid::~Solenoid()
{
    allocated->Free(SlotToIndex(m_chassisSlot) * kSolenoidChannels +
                    m_channel - 1);
    if (m_refCount == 1)
    {
...
{
    UINT32 value =
        m_fpgaSolenoidModule->readDO7_0(&status) & (1 << (m_channel - 1));
    wpi_assertCleanStatus(status);
    return (value != 0);
}
