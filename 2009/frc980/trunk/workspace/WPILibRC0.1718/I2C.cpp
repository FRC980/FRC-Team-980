...
void I2C::Write(UINT8 registerAddress, UINT8 data)
{
    semTake(semaphore, WAIT_FOREVER);
    {
        m_module->m_fpgaDIO->writeI2CConfig_Address(m_deviceAddress,
                                                    &status);
        m_module->m_fpgaDIO->writeI2CConfig_Register(registerAddress,
                                                     &status);
        m_module->m_fpgaDIO->writeI2CConfig_DataToSend(data, &status);
        m_module->m_fpgaDIO->writeI2CConfig_Read(false, &status);
        UINT8 transaction =
            m_module->m_fpgaDIO->readI2CStatus_Transaction(&status);
        m_module->m_fpgaDIO->strobeI2CStart(&status);
        while (transaction ==
               m_module->m_fpgaDIO->readI2CStatus_Transaction(&status))
            taskDelay(1);
        while (!m_module->m_fpgaDIO->readI2CStatus_Done(&status))
            taskDelay(1);
    }
    semGive(semaphore);
}
 ...
 */
bool I2C::VerifySensor(UINT8 registerAddress, UINT8 count,
                       const UINT8 * expected)
{
    UINT8 deviceData[4];
    for (UINT8 i = 0, curRegisterAddress = registerAddress; i < count;
