 ...
 *                     sourceforge.WPI.edu
 *  File Description : C Routines for FIRST Vision API. Open source API
 *                     developed by BAE Systems to interface with the
 *                     National Instruments vision C library in the
 *                     nivision.out module. The published interface to
 *                     nivision.out is in the header file nivision.h and
 *                     documented in the NIVisionCVI.chm help file.
*/
/*----------------------------------------------------------------------------*/
...
* The border size is defaulted to 3 so that convolutional algorithms work
* at the edges.  When you are finished with the created image, dispose of
* it by calling frcDispose().  To get extended error information, call
* GetLastError().
*
...
* @return On success: 1. On failure: 0. To get extended error information,
* call GetLastError().
*/
int frcDispose(void *object)
{
    return imaqDispose(object);
}

/**
* @brief Dispose of a list of objects. Supports any object created on the
* heap.
*
...
* IMAQ_IMAGE_HSL.
*
* @param dest Copy of image. On failure, dest is NULL. Must have already
* been created using frcCreateImage().  When you are finished with the
* created image, dispose of it by calling frcDispose().
...
/**
 * @brief Creates image object from the information in a file. The file
 * can be in one of the following formats: PNG, JPEG, JPEG2000, TIFF,
 * AIPD, or BMP.  Supports IMAQ_IMAGE_U8, IMAQ_IMAGE_I16, IMAQ_IMAGE_SGL,
 * IMAQ_IMAGE_COMPLEX, IMAQ_IMAGE_RGB, IMAQ_IMAGE_HSL, IMAQ_IMAGE_RGB_U64.
 *
 * @param image Image read in
 * @param fileName File to read. Cannot be NULL
 *
 * @return On success: 1. On failure: 0. To get extended error
 ...
 * @param fileName File to read. Cannot be NULL. The extension determines
 * the file format that is written.
 *
 * @return On success: 1. On failure: 0. To get extended error
 * information, call GetLastError().
 */
...
* Outputs intensity based statistics about an image such as Max, Min, Mean
* and Std Dev of pixel value.  Supports IMAQ_IMAGE_U8, IMAQ_IMAGE_I16,
* IMAQ_IMAGE_SGL.
*
...
*           min to 0 and the max to 255 for 8-bit images and to the actual
*           minimum value and maximum value of the image for all other
*           image types.
...
* @param numClasses The number of classes into which the function
* separates the pixels.  Determines the number of elements in the
* histogram array returned
...
* @param rect Region of interest in the image. If not included, the entire
* image is used.
* @return On success, this function returns a report describing the pixel
* value classification.  When you are finished with the report, dispose of
* it by calling frcDispose().  On failure, this function returns NULL. To
* get extended error information, call GetLastError().
*
*/
HistogramReport *frcHistogram(const Image * image, int numClasses,
                              float min, float max)
{
    Rect rect = IMAQ_NO_RECT;
...
/**
* @brief Calculates the histogram, or pixel distribution, of a color
* image.  Supports IMAQ_IMAGE_RGB, IMAQ_IMAGE_HSL.
*
* @param image Image whose histogram the function calculates.
* @param numClasses The number of classes into which the function
* separates the pixels.  Determines the number of elements in the
...
* image.  The function calculates the histogram using only those pixels in
* the image whose corresponding pixels in the mask are non-zero. Set this
* parameter to NULL to calculate the histogram of the entire image, or use
* the simplified call.
*
...
* finished with the report, dispose of it by calling frcDispose().  On
* failure, this function returns NULL.  To get extended error information,
* call imaqGetLastError().
*/
...
{
    return imaqColorHistogram2((Image *) image, numClasses, mode, NULL,
                               mask);
}
...
* @brief Measures the pixel intensities in a rectangle of an image.
* Outputs intensity based statistics about an image such as Max, Min, Mean
* and Std Dev of pixel value.  Supports IMAQ_IMAGE_U8 (grayscale)
* IMAQ_IMAGE_RGB (color) IMAQ_IMAGE_HSL (color-HSL).
*
...
* parameter cannot be NULL.  This data structure contains either
* grayscale, RGB, HSL, Complex or RGBU64Value depending on the type of
* image.
* @return On success: 1. On failure: 0. To get extended error information,
* call GetLastError().
*/
...
* Supports IMAQ_IMAGE_U8, IMAQ_IMAGE_I16, IMAQ_IMAGE_SGL.
*
* @param dest The destination image. If dest is used, it must be the same
* size as the Source image. It will contain only the filtered particles.
...
* source image. This array cannot be NULL.  See the NIVisionCVI.chm help
* file for definitions of criteria.
...
{
    Rect rect = IMAQ_NO_RECT;
    return frcParticleFilter(dest, source, criteria, criteriaCount, options,
                             rect, numParticles);
}
                      ...
                      int *numParticles)
{
    ...
    return imaqParticleFilter3(dest, source, criteria, criteriaCount,
                               options, roi, numParticles);
}
                        ...
                        ParticleAnalysisReport * par)
{
    ...
    double returnDouble;
    success =
        imaqMeasureParticle(image, particleNumber, 0,
    ...
    success =
        imaqMeasureParticle(image, particleNumber, 0, IMAQ_MT_AREA,
                            &returnDouble);
    if (!success)
    {
        return success;
    ...
    par->center_mass_x_normalized =
        RangeToNormalized(par->center_mass_x, width);
    par->center_mass_y_normalized =
        RangeToNormalized(par->center_mass_y, height);
...
* pixel values that are less than or equal to the Minimum in the original
* image are set to 0 for an 8-bit image. In 16-bit and floating-point
* images, these pixel values are set to the smallest pixel value found in
* the original image.
* @param max the largest value used for processing. After processing, all
* pixel values that are greater than or equal to the Maximum in the
* original image are set to 255 for an 8-bit image. In 16-bit and
* floating-point images, these pixel values are set to the largest pixel
* value found in the original image.
* @param mask an 8-bit image that specifies the region of the small image
* that will be copied. Only those pixels in the Image Src (Small) image
* that correspond to an equivalent non-zero pixel in the mask image are
* copied. All other pixels keep their original values. The entire image is
* processed if Image Mask is NULL or this parameter is omitted.
* @return On success: 1. On failure: 0. To get extended error information,
* call GetLastError().
*
*  option defaults:
                      ...
                      ObjectType type)
{
    ...
    return imaqLocalThreshold(dest, source, windowWidth, windowHeight,
                              method, deviationWeight, type, replaceValue);
}
                       ...
                       float rangeMax, float newValue)
{
    ...
    return imaqThreshold(dest, source, rangeMin, rangeMax, useNewValue,
                         newValue);
}
...
* @return On success: 1. On failure: 0. To get extended error information, call GetLastError().
*/
int frcHueThreshold(Image * dest, const Image * source,
                    const Range * hueRange)
{
    return frcHueThreshold(dest, source, hueRange,
                           DEFAULT_SATURATION_THRESHOLD);
}

int frcHueThreshold(Image * dest, const Image * source,
                    const Range * hueRange, int minSaturation)
{
    // assume HSL mode
...
{
    return frcExtractHuePlane(image, huePlane,
                              DEFAULT_SATURATION_THRESHOLD);
}
