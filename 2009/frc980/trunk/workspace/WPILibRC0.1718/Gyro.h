 ...
 * position.  The Gyro class tracks the robots heading based on the
 * starting position. As the robot rotates the new heading is computed by
 * integrating the rate of rotation returned by the sensor. When the class
 * is instantiated, it does a short calibration routine where it samples
 * the gyro while at rest to determine the default offset. This is
