 ...
 * center value for this part. Then use the center value as the
 * Accumulator center value for subsequent measurements.  It's important
 * to make sure that the robot is not moving while the centering
 * calculations are in progress, this is typically done when the robot is
 * first turned on while it's sitting at rest before the competition
 * starts.
 */
void Gyro::InitGyro()
{
 ...
 * This takes the number of volts/degree/second sensitivity of the gyro
 * and uses it in subsequent calculations to allow the code to work with
 * multiple gyros.
 *
 * @param voltsPerDegreePerSecond The type of gyro specified as the
 * voltage that represents one degree/second.
 */
