 ...
 */
int GetImageBlocking(Image * image, double *timestamp,
...
int GetImage(Image * image, double *timestamp)
{
    ...
    while (readCount)
    {
        ...
        if (!imaqDuplicate
            (image, globalCamera.data[readIndex].decodedImage))
        {
            int  errorCode = GetLastVisionError();
            DPRINTF(LOG_DEBUG, "Error duplicating image= %i  %s ",
        ...
        if (readIndex == globalCamera.index)
            break;
        readCount--;
    }
    ...
    if (currentTime >
        globalCamera.data[globalCamera.index].timestamp +
        CAMERA_IMAGE_STALE_TIME_SEC)
    {
        DPRINTF(LOG_CRITICAL,
 ...
 */
int GetImageData(char **imageData, int *numBytes,
                 double *currentImageTimestamp)
{
    int  success = 0;
    int  readIndex;
    ...
    while (readCount)
    {
        ...
        memcpy(cameraImageString, globalCamera.data[readIndex].cameraImage,
               cameraImageSize);
        *currentImageTimestamp = globalCamera.data[readIndex].timestamp;
        // make sure this buffer is not being written to now
        if (readIndex == globalCamera.index)
 ...
 * @return 0 if failure; 1 if success
 */
int GetImageDataBlocking(char **imageData, int *numBytes, double *timestamp,
                         double lastImageTimestamp)
{
 ...
 * @return 0 if failure; 1 if success
*/
static int CameraReadLine(int camSock, char *buffer, int bufSize,
                                                     ...
                                                     const char *request)
{
    ...
    static const int numAuthenticationStrings =
        sizeof(authenticationStrings) / sizeof(authenticationStrings[0]);
    ...
    for (; i < numAuthenticationStrings; ++i)
    {
        ...
        if (((int)
             (cameraAddr.sin_addr.s_addr =
              inet_addr(const_cast < char *>(serverName))) == ERROR)
            &&
            ((int)
             (cameraAddr.sin_addr.s_addr =
              hostGetByName(const_cast < char *>(serverName))) == ERROR))
        {
            imaqSetError(ERR_CAMERA_CONNECT_FAILED, funcName);
            return
                CameraCloseSocket("Failed to get IP, check hostname or IP",
                                  camSock);
        }
        ...
        {
            return CameraCloseSocket("Bad response to GET request",
                                     camSock);
        }
...
int ConfigureCamera(char *configString)
{
    ...
    camSock =
        CameraOpenSocketAndIssueAuthorizedRequest(serverName,
                                                  cameraRequest);
...
int GetCameraSetting(char *configString, char *cameraResponse)
{
    ...
    camSock =
        CameraOpenSocketAndIssueAuthorizedRequest(serverName,
                                                  cameraRequest);
    DPRINTF(LOG_DEBUG,
            "return from CameraOpenSocketAndIssueAuthorizedRequest %i",
            camSock);
                     ...
                     ImageRotation rotation)
{
    ...
    case k640x480:
        {
            sprintf(resStr, "640x480");
            break;
        }
    case k320x240:
        {
            sprintf(resStr, "320x240");
            break;
        }
    case k160x120:
        {
            sprintf(resStr, "160x120");
            break;
        }
    default:
        {
            DPRINTF(LOG_DEBUG, "code error - resolution input");
            break;
        }
    }
    ...
    char *getStr1 = "GET /axis-cgi/mjpg/video.cgi?showlength=1&camera=1&";

    char insertStr[100];
    sprintf(insertStr,
            "des_fps=%i&compression=%i&resolution=%s&rotation=%i", frames,
            compression, resStr, (int)rotation);
...
Authorization: Basic %s;\n\n";
  ...
  RETRY:
    ...
    if (!authorizeConfirmed)
    {
        ...
        {
            sprintf(tempBuffer, getStr,
                    authenticationStrings[authorizeCount]);
        }
        else
        {
            imaqSetError(ERR_CAMERA_AUTHORIZATION_FAILED, funcName);
    ...
    while (1)
    {
        ...
        if (((int)(cameraAddr.sin_addr.s_addr = inet_addr(serverName)) ==
             ERROR)
            &&
            ((int)(cameraAddr.sin_addr.s_addr = hostGetByName(serverName))
             == ERROR))
        {
            CameraCloseSocket("Failed to get IP, check hostname or IP",
                              camSock);
        ...
        {
            imaqSetError(ERR_CAMERA_CONNECT_FAILED, funcName);
            CameraCloseSocket("Failed to connect to camera - check network",
                              camSock);
            continue;
        }
...
#define MEASURE_TIME 0
    ...
    while (cont)
    {
        ...
        while (counter)
        {
            ...
            if (NULL != strstr(trailingPtr, "\r\n\r\n"))
            {
                ...
                if (!authorizeConfirmed)
                {
                    ...
                    {
                        CameraCloseSocket
                            ("Not authorized to connect to camera",
                             camSock);
                        authorizeCount++;
                        goto RETRY;
                    }
        ...
        {
            globalCamera.cameraMetrics[ERR_CAMERA_HEADER_ERROR]++;
            CameraCloseSocket("No content-length token found in packet",
                              camSock);
            goto RETRY;
        }
        ...
        globalCamera.data[writeIndex].cameraImage =
        ...
        if (globalCamera.decode)
        {
            ...
            globalCamera.data[writeIndex].decodedImage =
                frcCreateImage(IMAQ_IMAGE_HSL);
            if (!Priv_ReadJPEGString_C
                (globalCamera.data[writeIndex].decodedImage,
                 (const unsigned char *)globalCamera.data[writeIndex].
                 cameraImage,
                 globalCamera.data[writeIndex].cameraImageSize))
            {
        ...
        if (globalCamera.readerPID)
        {
            if (taskKill(globalCamera.readerPID, SIGUSR1) == OK)
                DPRINTF(LOG_DEBUG, "SIGNALING PID= %i",
                        globalCamera.readerPID);
            else
                globalCamera.cameraMetrics[CAM_PID_SIGNAL_ERR]++;
            DPRINTF(LOG_DEBUG, "ERROR SIGNALING PID= %i",
                    globalCamera.readerPID);
        }
        ...
        {
            DPRINTF(LOG_DEBUG, "imageCounter = %i",
                    globalCamera.cameraMetrics[CAM_NUM_IMAGE]);
            printCounter = 0;
        }
        ...
        if (performanceLoopCounter <= maxCount)
        {
            DPRINTF(LOG_DEBUG,
                    ...
                    performanceLoopCounter, loopEndTime - loopStartTime,
                    cameraEndTime - cameraStartTime,
                    (loopEndTime - loopStartTime) - (cameraEndTime -
                                                     cameraStartTime),
...
{
    globalCamera.cameraMetrics[CAM_STOPS]++;
    globalCamera.acquire = 0;
}
                    ...
                    ImageRotation rotation)
{
    ...
    // spawn camera task
    bool started =
        g_axisCameraTask.Start(frames, compression, resolution, rotation);
