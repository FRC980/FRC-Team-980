...
void Encoder::InitEncoder(bool reverseDirection, EncodingType encodingType)
{
    ...
    {
    case k4X:
        Resource::CreateResourceObject(&quadEncoders,
                                       tEncoder::kNumSystems);
        //TODO: need to check for errors here
        m_index = quadEncoders->Allocate();
        m_encoder = new tEncoder(m_index, &status);
        m_encoder->writeConfig_ASource_Module(m_aSource->
                                              GetModuleForRouting(),
                                              &status);
        ...
        m_encoder->writeConfig_ASource_AnalogTrigger(m_aSource->
                                                     ...
                                                     (), &status);
        m_encoder->writeConfig_BSource_Module(m_bSource->
                                              GetModuleForRouting(),
                                              &status);
    ...
    case k2X:
        m_counter =
            new Counter(m_encodingType, m_aSource, m_bSource,
                        reverseDirection);
        break;
    }
