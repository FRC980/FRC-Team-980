...
typedef enum
{ kOff, kOn, kForward, kReverse } RelayValue;
typedef enum
{ kBothDirections, kForwardOnly, kReverseOnly } RelayDirection;

void InitRelay(UINT32 channel, RelayDirection direction = kBothDirections);
void InitRelayRelay(UINT32 slot, UINT32 channel, RelayDirection direction =
                    kBothDirections);
