...
class SmartDrive : public SpeedController
{
  private:
     ...
     *
     * Call the Calculate method as a non-static method. This avoids
     * having to prepend all local variables in that method with the class
     * pointer. This way the "this" pointer will be set up and class
     * variables can be called more easily.  This method is static and
     * called by the Notifier class.
     *
     * \param controller the address of the PID controller object to use
     ...
     *  minimize slippage, (as measured by the difference between the
     *  motor speed and follow wheel speed).
     */
    void Calculate(void);
  ...
  public:
     ...
     */
    SmartDrive(uint8_t id, double kvp, double kvi, // velocity PID constants
               double kcp, double kci, // correction PID constants
               double kap, double kai, bool kbacl, // accel PID constants
               SpeedController * psc,
               Encoder * pEncDrive,
               Encoder * pEncFollow, float period = 0.010);
