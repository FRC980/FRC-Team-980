...
void SmartDrive::Calculate(void)
{
    ...
    double dMotorVel =
        (dMotorCount - m_dPrevMotorCount) / m_kfPeriod / TOP_SPEED;
    double dRobotVel =
        (dRobotCount - m_dPrevRobotCount) / m_kfPeriod / TOP_SPEED;
    ...
    if (!Main::getInstance().IsDisabled())
    {
        ...
        if (!m_ku8ID ? (DEBUG_bOK0 && DEBUG_bOK1)
            : (DEBUG_bOK1 && !DEBUG_bOK0))
        {
    ...
    if (m_bEnabled)
    {
        ...
        // m_dCorInt = utils::limit(m_dCorInt, -1.25, 1.25);
